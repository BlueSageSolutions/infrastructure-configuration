...
COMMONOPTS="-server -Djdk.tls.client.protocols=TLSv1.2 -Dhttps.protocols=TLSv1.2 -Djava.security.egd=file:///dev/./urandom -Dsun.net.http.retryPost=false"
...
if [[ $? -eq 0 ]]; then
    echo "Using Grails 2"
    JVM=$JVM2
    OPTS="-Xms2g -Xmx4g -XX:PermSize=256m -XX:MaxPermSize=1024m -XX:+UseG1GC"
